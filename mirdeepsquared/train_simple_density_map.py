...
from .common import prepare_data, split_data_once, to_xy_with_location
...
def train_density_map(df, epochs=200):
    ...
    X_train_features = [location_of_mature_star_and_hairpin, density_maps]
    X_val_features = [X_val[1], X_val[2]]
    ...
    dense_layer = Dense(100, activation='relu', kernel_initializer=HeNormal(seed=42), use_bias=True, bias_initializer=RandomNormal(mean=0.0, stddev=2.5, seed=42))(flatten_layer_structure)
    ...
    history = model.fit(X_train_features, Y_train, epochs=epochs, batch_size=16, class_weight=class_weights_dict, validation_data=(X_val_features, Y_val), callbacks=[early_stopping])  # verbose=0
